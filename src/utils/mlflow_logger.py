...
class MLflowLogger:
    ...
    def initialize_tracking(cls, experiment_name: str = "ML Experiments"):
        """Configura MLflow para usar 'data/mlflow_tracking/mlruns' (idempotente)."""
        if cls._tracking_initialized:
            return
        
        # Encontra a raiz do projeto: src/utils/../.. = projeto root
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        mlruns_path = os.path.join(project_root, "data", "mlflow_tracking", "mlruns")
        ...
        try:
            ...
            print(f"✅ MLflow configurado para: {mlruns_path}")
        except Exception as e:
            print(f"⚠️ Aviso ao configurar experimento: {e}")
    ...
    def log_run(
        ...
        with mlflow.start_run(run_name=run_name):
            ...
            mlflow.set_tags({
                ...
                **{k: experiment_metadata.get(k) for k in ["experiment_description", "model_description", "run_description"] if experiment_metadata.get(k)}
            })
            ...
            # Data Lineage
            mlflow.log_input(mlflow.data.from_pandas(X_train_df, name=f"{model_name}_train"), "training")
            mlflow.log_input(mlflow.data.from_pandas(X_test_df, name=f"{model_name}_test"), "testing")

            # Gera plots
            cls._create_and_log_plots(y_test, y_pred_test, y_prob_test, importance_df, model_name, show_plots)
            ...
            description = f"Modelo de Rede Neural para Previsão de Churn.\n\n{str(model)}" if is_torch else f"Modelo de ML para Previsão de Churn.\n\n{str(model.__class__.__name__)}"
        ...
        print(f"🚀 [MLflow] Execução registrada com sucesso em 'data/mlflow_tracking'!")
    
    @classmethod
    def _create_and_log_plots(cls, y_test, y_pred_test, y_prob_test, importance_df, model_name, show_plots):
        ...
        if importance_df is not None:
            ...
            top_features = importance_df.head(15).sort_values(by="absolute_importance", ascending=True)
